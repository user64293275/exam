...
const Munchen = () => {
  ...
  useEffect(() => {
      const currentWeather = async (url) => {
        ...
        hourlyForecast(
        `https://api.openweathermap.org/data/2.5/forecast?q=Munich&appid=${API_KEY}=metric`
        );
        currentWeather(
            `https://api.openweathermap.org/data/2.5/weather?q=Munich&appid=${API_KEY}=metric`
        );
        forecast5Day(
            `https://api.openweathermap.org/data/2.5/forecast?q=Munich&appid=${API_KEY}=metric`
        );
    ...
    const groupByDay = (forecastData) => {
        ...
        forecastData.forEach((item) => {
          ...
          groupedData[date].weatherDescriptions.push(item.weather.description);
  ...
  return (
    <main className="main h-[95vh] bg-black justify-center mt-4 items-center mx-4  bg-opacity-50  md:flex md:mt-20 md:mx-16 md:h-[80vh]">
      ...
      <div className="">
        ...
        <div className="w-100 h-100 bg-black bg-opacity-55 rounded-2xl p-2">
            <YMaps>
              <Map
                defaultState={{
                  center: [48.13156777222954,11.574311999999969],
                  zoom: 12,
                }}
              ...
              >
                <Placemark
                  geometry={[48.13156777222954,11.574311999999969]}
                  properties={{
                    hintContent: "Munich",
                    balloonContent: "City of German",
                  }}
                />
              </Map>
            </YMaps>
