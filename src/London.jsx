...
const London = () => {
  ...
  useEffect(() => {
    ...
    hourlyForecast(
      `https://api.openweathermap.org/data/2.5/forecast?q=London&appid=${API_KEY}=metric`
    );
    currentWeather(
      `https://api.openweathermap.org/data/2.5/weather?q=London,uk&appid=${API_KEY}=metric`
    );
    forecast5Day(
      `https://api.openweathermap.org/data/2.5/forecast?q=London&appid=${API_KEY}=metric`
    );
  ...
  return (
    <main className=" h-[95vh] bg-black justify-center mt-4 items-center mx-4  bg-opacity-50 flex md:mt-20 md:mx-16 md:h-[80vh]">
      ...
      <div className="">
        <div className="bg-black bg-opacity-55 my-5 rounded-2xl">
          ...
          <div className="bg-black bg-opacity-55 my-5 rounded-2xl ">
            ...
            <div className="border-t-[1px] border-gray-700 flex">
              {forecastState &&
                Object.keys(forecastState).map((date) => (
                  <div
                    key={date}
                    className="my-3 mx-3 p-1 rounded-lg hover:bg-[#495057]"
                  >
                    <h4 className="text-sm text-white font-semibold">{date}</h4>
                    <h5 className="text-sm text-[#0081a7]">{forecastState[date].weatherDescriptions[4]}</h5>
                    <h3 className="text-lg text-white font-semibold">
          ...
          <div className="w-100 h-[350px] bg-black bg-opacity-55 rounded-2xl p-2">
              <YMaps>
                <Map
                  defaultState={{
                    center: [51.50167280981746,-0.09968971435546754],
                    zoom: 12,
                  }}
                ...
                >
                  <Placemark
                    geometry={[51.50167280981746,-0.09968971435546754]}
                    properties={{
                      hintContent: "London",
